...
getData() {
	...
	colorEcho ${BLUE}  " Trojan密码: $PASSWD
}
...
getConfigFileInfo() {
    ...
    passwd=`grep password $CONFIG_FILE | head -n1| cut -d: -f2 | tr -d \",' '`
...
outputtrojan() {
    ...
    echo -e "   ${BLUE}密码：${PLAIN}${RED}${passwd}${PLAIN}"
    echo -e "   ${BLUE}传输协议：${PLAIN} ${RED}${network}${PLAIN}"
}
